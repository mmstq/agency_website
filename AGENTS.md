...
- **No Framer Motion / Three.js** — all animations use `requestAnimationFrame` + `useRef` DOM mutation (see [`AnalyticsCard.tsx`](src/components/AnalyticsCard.tsx:6), [`FeatureCardsStack.tsx`](src/components/FeatureCardsStack.tsx:6), [`CanvasGrid.tsx`](src/components/CanvasGrid.tsx:4)). Do NOT install animation libraries. **One sanctioned exception:** `three` is installed solely for [`Ballpit.jsx`](src/components/Ballpit.jsx) (interactive 3D ball-pit on the `/404` page), vendored from React Bits. Do NOT remove `three` or `Ballpit` thinking they violate this rule, and do NOT reach for Three.js anywhere else — Ballpit is the only permitted use.
